...
CONTEXT_BUDGET_PRESETS = {
    ...
    "256k": {
        ...
        "max_chars_per_conversation": 32_000,
        "max_total_chars": 320_000,
        "request_timeout": 360,
    },
    "1m": {
...
DEFAULT_EXTRACTION_MODEL = "moonshotai/kimi-k2.5"
DEFAULT_JUDGE_MODEL = "anthropic/claude-sonnet-4.5"


def get_tiered_model_choices() -> List[Tuple[str, str]]:
    """Return model choices with tier group headers for Gradio dropdowns."""
    choices: List[Tuple[str, str]] = []
    for tier_key, tier in MODEL_TIERS.items():
        label = tier["label"]
        for model in tier["models"]:
            choices.append((f"[{label}] {model}", model))
    return choices
