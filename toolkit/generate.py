...
def generate_with_llm(
    ...
    per_chat_input_budget = max(900, min(int(usable_context * 0.75), 12000))
    synthesis_input_budget = max(1200, min(int(usable_context * 0.85), 18000))
    if log_fn:
        log_fn(
...
def extraction_shell(companion_name: str) -> Dict[str, Any]:
    return {
        ...
        "tags": [],
        "voice_profile": {
            "cadence": "", "linguistic_markers": [], "emotional_style": "",
            "relational_contract": "", "evidence_snippets": [],
        },
        "memories": [],
    }
...
def build_ccv3_card(
    ...
    if isinstance(voice_profile, dict):
        cadence = _safe_text(voice_profile.get("cadence"))
        emotional_style = _safe_text(voice_profile.get("emotional_style"))
        if cadence or emotional_style:
            creator_notes = (
    ...
    card_data: Dict[str, Any] = {
        ...
        "description": _safe_text(draft.get("description")) or f"{companion_name} reconstructed from transcript evidence.",
        "tags": tags,
        "creator": _safe_text(creator, "unknown"),
        "character_version": "1.0",
        "mes_example": _safe_text(draft.get("mes_example")) or "<START>\n{{user}}: How are you?\n{{char}}: I'm here with you.",
        "extensions": {},
